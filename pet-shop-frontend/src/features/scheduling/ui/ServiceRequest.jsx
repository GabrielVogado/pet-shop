...
import { CalendarClock, Check, Loader2, Store, Syringe, Waves } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function loadInitialAvailability() {
      ...
      try {
        ...
        if (!cancelled) {
          setAvailabilityError('Não foi possível carregar agenda disponível.');
          setAvailableDates([]);
          setAvailableTimes([]);
          setSelectedDate('');
          setSelectedTime('');
  ...
  useEffect(() => {
    ...
    async function loadTimesForDate() {
      ...
      try {
        ...
        if (!cancelled) {
          setAvailabilityError('Não foi possível carregar horários disponíveis.');
          setAvailableTimes([]);
          setSelectedTime('');
        }
      } finally {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-ocean">Solicitação</p>
          <h2 className="mt-1 text-2xl font-bold text-ink sm:text-3xl">
            Escolha um serviço para {pet.name}
          </h2>
        </div>
        ...
        <div className="grid w-full grid-cols-2 rounded-md border border-ink/10 bg-white p-1 shadow-sm sm:w-auto">
          <button
            ...
            className={`flex min-h-11 items-center justify-center gap-2 rounded px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 ${
        ...
        <section className="rounded-md border border-ink/10 bg-white p-4 shadow-soft">
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block">
              <span className="text-sm font-bold text-ink/70">Dia disponível</span>
              <select
                value={selectedDate}
            ...
            <label className="block">
              <span className="text-sm font-bold text-ink/70">Horário disponível</span>
              <select
                value={selectedTime}
          ...
          {availabilityError && (
            <p className="mt-3 text-sm font-semibold text-coral" role="alert">{availabilityError}</p>
          )}
        </section>
      )}
      ...
      {noPetshops && (
        <div className="rounded-md border border-ink/10 bg-white p-6 text-center text-sm text-ink/55 shadow-sm">
          Nenhum petshop disponível no momento.
        </div>
      )}

      {!noPetshops && visibleServices.length === 0 && (
        <div className="rounded-md border border-ink/10 bg-white p-6 text-center text-sm text-ink/55 shadow-sm">
          Este petshop ainda não cadastrou {serviceType === 'baths' ? 'banhos' : 'vacinas'}.
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="mt-6 flex min-h-11 w-full items-center justify-center gap-2 rounded bg-coral px-4 text-sm font-bold text-white transition hover:bg-coral-hover focus:outline-none focus:ring-2 focus:ring-coral focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-ink/20 disabled:text-ink/45"
              >
