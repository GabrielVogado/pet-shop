...
import { OwnerServices } from '../../service-catalog/ui/OwnerServices';
	...
	return (
		<div className="min-h-screen bg-paper">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-sm text-ink/60">
								Dono: <span className="font-semibold text-ink">{owner.name}</span>
							</p>
						</div>
					</div>
			...
			<main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
				<section className="grid w-full grid-cols-2 rounded-md border border-ink/10 bg-white p-1 shadow-soft md:w-[520px]">
					<button
						type="button"
										...
										{appointments.length === 0 ? (
											...
											appointments.map((appointment) => {
												...
												return (
													...
													>
														<TableCell>{formatDateTime(appointment.dateTime ?? appointment.date)}</TableCell>
														<TableCell>
	...
	return (
		<aside className="space-y-4 rounded-md border border-ink/10 bg-white p-5 shadow-soft">
			...
			<InfoGroup icon={CalendarCheck} title="Agendamento">
				<InfoLine label="Data/Hora" value={formatDateTime(appointment.dateTime ?? appointment.date)} />
